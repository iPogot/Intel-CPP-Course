...
public:
    ...
    Line(const Point p1, const Point p2): v_(p2, p1), p0_(p1) {}

    Line(const Plane p1, const Plane p2): v_(p1.get_norm() * p2.get_norm()) {
    ...
    double get_parameter(const Point p) {
        ...
        // v_.print_coordinates();
        // p0_.print_coordinates();
        if(v_.get_x()) {
    ...
    bool find_intersection(const Line l, Point& p) {
        ...
        if( compare_doubles(v_.get_z(), 0) && compare_doubles(l.get_vector().get_z(), 0) ) { //xy
            ...
            if(!compare_doubles(det, 0)) {
                ...
                p = Point(d0*b1 - b0*d1 ,0 , a0*d1 - a1*d0);
                return true;
            ...
            if(!compare_doubles(det, 0)) {
                ...
                p = Point(0 ,d0*b1 - b0*d1 , a0*d1 - a1*d0);
                return true;
